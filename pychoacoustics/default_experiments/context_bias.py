...
def initialize_context_bias(prm):
    ...
    :return:
    """
    exp_name = "Contextual Bias Tritone"
...
def doTrial_context_bias(parent):
    ...
    rand_st = np.random.randint(12)
    fb1 = 2.**(rand_st/12.)  # base frequency chosen randomly uniformly on log-octave range
    ...
    for i in range(nb_tones_clear):
        ...
        tmp_st = ConstantIntervalChord(fb=fb1*2.**(st/12.),interval=np.sqrt(2.), env=genv, delay=run_time, duration=duration_sp_ctx)
...
# TODO
# - read:http://pychoacoustics.readthedocs.org/en/latest/psychophysics.html#available-paradigms
# - read:http://pychoacoustics.readthedocs.org/en/latest/adding_experiments.html#the-experiment-opts
